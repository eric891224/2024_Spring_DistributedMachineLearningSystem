...
def get_fixed_sample_fn(num_clients: int = 50, num_samples: int = 10000):
    def fixed_sample_fn(cid: int):
        return num_samples / num_clients

    return fixed_sample_fn


def alpaca_format_preprocessor(dataset: Dataset):
    return dataset.map(
        alpaca_format,
        remove_columns=["input", "output"],
        desc=f"Preprocessing {dataset.builder_name} for unified format.",
    )
...
def get_dataloaders(
    ...
    partitionner = SizePartitioner(
        num_partitions=cfl_args.num_clients,
        partition_id_to_size_fn=get_fixed_sample_fn(
            num_clients=cfl_args.num_clients, num_samples=dataset_args.dataset_samples
        ),
    )
    fds = FederatedDataset(
        dataset=dataset_args.dataset_name,
        preprocessor=alpaca_format_preprocessor,
        partitioners={"train": partitionner},
    )
    ...
    for partition_id in range(cfl_args.num_clients):
        ...
        trainloaders.append(DataLoader(partition["train"], batch_size=model_args.batch_size))
